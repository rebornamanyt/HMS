...
public class deletereceptionist extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					deletereceptionist frame = new deletereceptionist();
	...
	public deletereceptionist() {
		...
		JLabel lblNewLabel = new JLabel("Delete Receptionist");
	    ...
	    lblDelete.addMouseListener(new MouseAdapter() {
	    	...
	    	public void mouseClicked(MouseEvent e) {
	    		...
	    		String id,sql;
        		id=txtid.getText();
        		sql="UPDATE receptionist SET status='1' where id='"+id+"' ";
        		try {
	    ...
	    lblDelete.setBounds(924, 121, 116, 39);
	...
	private void datafound(){
		...
		String sql1="SELECT * FROM receptionist where id='"+id+"'";
		try {
			Statement stmt=dbc.conmethod();
			ResultSet rs=stmt.executeQuery(sql1);
			while (rs.next()){
